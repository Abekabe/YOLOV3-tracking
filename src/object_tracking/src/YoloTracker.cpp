    ...
    if (!init) {
        ...
        for (int num = 0; num < indexVector.size(); num++) {
            if (indexVector[num] != -1)
                if (lossMatrix[num][indexVector[num]] >= 100)
                    indexVector[num] = -1;
        }
    }
    return indexVector;
...
vector<double> YoloTracker::getLossVector(vector<double>& yoloDetection, vector<vector<double> >& predictMatrix) {
    ...
    for (int num = 0; num < predictMatrix.size(); num++) {
        double distance = sqrt(pow(predictMatrix[num][0] - yoloDetection[0], 2) + pow(predictMatrix[num][1] - yoloDetection[1], 2));
        double boxErr = sqrt(pow(predictMatrix[num][2] - yoloDetection[2], 2) + pow(predictMatrix[num][3] - yoloDetection[3], 2));
...
void YoloTracker::pubImage(const sensor_msgs::ImageConstPtr& msg){
  {
    ...
    {   
        ...
        for (int num = 0; num < objectVector.size(); num++) {
            if (objectVector[num].tracker.getFoundCount() < 5)
                continue;
